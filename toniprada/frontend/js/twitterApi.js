window.twitterApi = (function () {
    ...
    var getTweetsForUsersThrottled = throttleFunction(function (userNames, callback) {
        ...
        if (userNames.length == 0)
            ...
            var url = "http://search.twitter.com/search.json?callback=?&rpp=100&lang=es&q=";
            for (var i = 0; i < userNames.length; i++)
                url += userNames[i] + (i < userNames.length - 1 ? " OR " : "");
		//console.log("EN TWITTER, url: " + url);
            $.ajax({
                url: url,
                dataType: "jsonp",
                success: function (data) { 
					...
					console.log(data);
					callback($.grep(data.results || [], function (tweet) { return !tweet.to_user_id; }));
				}
